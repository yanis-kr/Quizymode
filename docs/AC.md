...
- **AC 2.1.1** [Authenticated] **Given** I am creating or editing an item, **when** I set the category, **then** I must choose both **navigation keyword rank 1** (primary topic) and **rank 2** (subtopic) from dropdowns populated for that category; I may select existing keywords or create my own **private** keyword by typing a name. The chosen rank1 and rank2 are sent as `navigationKeyword1` and `navigationKeyword2` and stored as the item’s **NavigationKeywordId1** and **NavigationKeywordId2**; they are also included in item keywords on save. Existing selections come from the navigation API; custom names are stored as private keywords.
...
**Data source:** Categories, navigation keywords (rank-1 and rank-2 with parent-child relationships), and the user's collections are loaded from the same APIs used elsewhere (e.g. `GET /categories`, `GET /keywords?category=...`, `GET /collections`). The UI should use cached data (e.g. React Query or equivalent) so that dropdowns and suggestions are populated from in-memory cache where available.
...
- **AC 2.2.6.2** [Authenticated] **Given** I have selected a category, **when** the page shows keyword controls, **then** I see **Primary topic (rank 1)** and **Subtopic (rank 2)** selectors with a short, non-technical explanation: e.g. "Primary topic is the main subject under this category (e.g. a language or exam name). Subtopic narrows it further (e.g. a specific unit or section). These help you and others find your items in the app." Options are populated from the navigation keywords API for that category (rank-1 list; rank-2 list when a rank-1 is selected), using cached data where available.
- **AC 2.2.6.3** [Authenticated] **Given** I cannot find a suitable primary topic or subtopic in the lists, **when** I use the Bulk Create page, **then** I can add a **new private keyword** by typing a name (e.g. in a "Custom primary topic" or "Custom subtopic" field). New private keywords are validated: **alphanumeric and hyphens only** (spaces disallowed; use a hyphen as separator, e.g. `my-topic`), and **basic profanity filter** applied; invalid or blocked names show a clear validation message. These custom keywords are not created in the backend until I confirm import (accept/accept all).
...
- **AC 3.6.3** [Anyone] **Given** I am on **Flashcards** or **Quiz** for a **category** scope, **when** I view the scope secondary bar, **then** it shows **Sets | List Items | Flashcards | Quiz** (same as on Sets/List Items); clicking **Sets** navigates back to the Sets view for that scope. For **collection** scope, the bar shows only **List Items | Flashcards | Quiz** (no Sets).
