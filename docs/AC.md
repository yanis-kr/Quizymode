...
- **AC 3.0.7** [Anyone] **Given** I am anywhere in the SPA, **when** the shared page chrome is visible, **then** I see a footer with **Feedback**, **Ideas**, **About**, **Privacy**, **Terms**, and **User Guide** actions; **Ideas** opens `/ideas`, **About** opens `/about`, and **User Guide** opens the desktop or mobile guide based on the current browser.
...
- **AC 7.1.0** [Dev tooling] Four user-guide wrapper pairs exist. `generate-user-guides-production.ps1/.sh` captures both desktop and mobile screenshots from `https://www.quizymode.com` (no local stack required) and rebuilds both `docs/user-guide/user-guide.md` and `docs/user-guide/user-guide.mobile.md`. `generate-user-guides-local.ps1/.sh` does the same against `http://localhost:7000` using the same server-management pattern as the local E2E runners. The existing `generate-user-guide-production.ps1/.sh` and `generate-user-guide-local.ps1/.sh` remain as desktop-only variants.
...
- **AC 7.1.2** [Dev tooling] Running `node scripts/generate-user-guide.js --guide desktop` reads PNGs from `docs/user-guide/screenshots/user/` and writes `docs/user-guide/user-guide.md`; `--guide mobile` reads from `docs/user-guide/screenshots/mobile/` and writes `docs/user-guide/user-guide.mobile.md`; `--all` rebuilds both guides in one command. The mobile guide uses the same section order and screenshot slugs as desktop but can provide mobile-specific descriptive copy for narrow-screen UX differences.
