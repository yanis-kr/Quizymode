...
- **AC 2.1.1** [Authenticated] **Given** I am creating or editing an item, **when** I use the form, **then** I see a single **Topic and tags** section containing **Category** *, **Primary topic (rank 1)** *, **Subtopic (rank 2)** *, and **Additional keywords** (optional tag chips). I must choose both primary topic and subtopic from dropdowns populated for that category (or create my own **private** keyword by typing a name). Custom navigation names use the same format rules as elsewhere (letters, numbers, hyphens; max length). The chosen rank1 and rank2 are sent as `navigationKeyword1` and `navigationKeyword2` and stored as the item’s **NavigationKeywordId1** and **NavigationKeywordId2**; they are also included in item keywords on save. Existing selections come from the navigation API; custom names are stored as private keywords.
...
- **AC 2.2.6.2** [Authenticated] **Given** I have selected a category, **when** the page shows keyword controls inside the **Topic and tags** section, **then** I see **Primary topic (rank 1)** and **Subtopic (rank 2)** as **required** fields (same pattern as single-item create/edit), with a short explanation that both narrow where items appear. Options are populated from the navigation keywords API for that category (rank-1 list; rank-2 list when a rank-1 is selected), using cached data where available. I cannot generate a prompt or save bulk items without both rank-1 and rank-2.
