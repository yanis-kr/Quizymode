...
- **AC 3.0.7** [Anyone] **Given** I am anywhere in the SPA, **when** the shared page chrome is visible, **then** I see a footer with a **Feedback** action, a **Categories Map** action, an **About** action, and a **User Guide** action; **About** opens `/about`, and **User Guide** opens the desktop or mobile guide based on the current browser.
...
- **AC 3.0.7b** [Anyone] **Given** I am on `/categories` (root or drilled-in), **when** the filter control bar is visible, **then** I see a **Map** button to the right of the Filters button (and to the left of the Sort control when present); clicking it opens the same Categories Map modal as the footer action. On the root `/categories` page (bucket grid), a standalone Map button is shown above the grid. No additional vertical row is added; the button occupies horizontal space within the existing control bar row.
...
- **AC 3.0.11** [Anyone] **Given** I open `/categories`, `/collections`, or `/items/add`, **when** the page renders, **then** it uses the dark blue theme (dark background, light text, dark card containers with subtle borders) consistent with the Home page — no white wrapper is applied by the Layout for these routes.
