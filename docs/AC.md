...
- **Taxonomy item-tag slugs**
  - Any keyword name listed in a category’s taxonomy definition (`docs/quizymode_taxonomy.yaml`, including every L1 and L2 slug in that category) is treated as a **public** tag when attached to items: the API resolves or creates a non-private keyword (seeded by the taxonomy SQL as repo-managed public content) instead of a user-private, review-pending keyword.
...
| **Scope** | The **list of items** returned in a given context. Scope is determined by: (1) **Category/keywords** — navigation (path) and optional filters (query, scope filters) on the Categories page; or (2) **Collection** — items in a specific collection (collection-scoped visibility). The same scope can be viewed in **List**, **Flashcards**, or **Quiz** (and in Categories, also **Sets**). |
...
| **Seed set** | A logical label for a repo-managed item scope, typically derived from the GitHub-loaded seed-source path (for example `data/seed-source/items/sports`). It is returned in preview/apply responses for context, but per-item identity is the permanent `itemId`. |
| **Repo-managed item** | A public item whose content is controlled by source control and the admin seed-sync process, identified by a permanent repo-managed `itemId` that matches `Items.Id`. Repo-managed rows are marked with `IsRepoManaged = true`; the item row itself does not store extra seed metadata columns. |
...
- **AC 1.2.9** [Owner] **Given** I am on the Collections page (**Mine** tab), **when** I use the "Set active" control on a collection card, **then** that collection becomes my active collection (used for add/remove on items); the current active collection is indicated on the card (e.g. checkmark or filled icon).
- **AC 1.2.10** [Owner] **Given** I am authenticated and have an active collection, **when** I visit the Add Items hub (`/items/add`), Create Item (`/add-new-item`), or Bulk Create (`/items/bulk-create`) pages, **then** I see an informational banner identifying my active collection by name. The banner warns that newly created items will be added to that collection, and if the collection is public (or made public in the future), those items will be accessible via its shareable link. If the collection is currently public the banner highlights this prominently.
- **AC 1.2.11** [Owner] **Given** I successfully create one or more items (single-item create or bulk create accept), **when** the creation succeeds, **then** the new item(s) are automatically added to my active collection (fire-and-forget; failure does not block or surface an error to the user).
...
- **AC 1.5.8** [Anyone] **Given** I am in **Flashcards** mode for a **collection** scope, **when** I study an item, **then** the Flashcards interaction matches **AC 3.6.2a** (question face-up; click to flip to answer and explanation; click again to flip back; reset to question face-up on previous/next item).
...
- **AC 1.7.4** [Owner] **Given** I am on the Collections **list** page (**Mine** tab), **when** I use the Edit control on a collection card, **then** a modal opens where I can change name, description, and "Shared with others"; saving persists changes per API and closes the modal.
...
- **AC 2.1.2** [Authenticated] **Given** I am creating or editing an item (any user, including admin), **when** the item form is shown, **then** I see fields for question, answers, explanation, source, navigation keywords, and simple tags. The form does **not** include Factual Risk or Review Comments inputs; those fields are internal and not surfaced in the item create/edit UI or API.
- **AC 2.1.3** [Admin] **Given** I am creating or editing an item as an **admin**, **when** the item form is shown, **then** I see the same fields as regular users, plus the ability to toggle `IsPrivate` and set `ReadyForReview`. Factual Risk and Review Comments are not exposed in this form.
...
**Data source:** Taxonomy structure for category and L1/L2 navigation comes from **`GET /taxonomy`** (cached for the session). Category and Sets browse still use **`GET /categories`** and **`GET /keywords`** where needed, and the taxonomy payload remains the canonical source for navigation shape. The user's collections come from **`GET /collections`**. The UI should use cached data (e.g. React Query or equivalent) so taxonomy-backed dropdowns minimize round trips.
...
- **AC 2.2.6.6** [Authenticated] **Given** I have selected category and at least primary topic (rank 1), **when** I click **Generate Prompt**, **then** the app shows an optimized prompt text for **any AI assistant** (e.g. ChatGPT, Claude). The prompt asks for up to **15 questions** in JSON and includes the selected scope directly in each item shape: `category`, `navigationKeyword1`, `navigationKeyword2`, `question`, `correctAnswer`, `incorrectAnswers`, optional explanation/source, and optional extra `keywords`. The prompt tells the model that `navigationKeyword1` and `navigationKeyword2` must exactly match the selected taxonomy path, asks the model to optionally include up to **five** extra **keywords** per item (slug-style tags) that do not duplicate my navigation topic or my comma-separated extra tags, and asks the model to provide a **direct source URL** per item when possible using a **reliable, verifiable reference** rather than the AI assistant name; the prompt wording refers to "your AI assistant" or "any AI assistant", not a specific product. The category and keywords I selected are baked into the prompt so the AI generates items for that scope.
...
- **AC 2.4.1** [Anyone] **Given** an item exists, **when** I call `GET /ratings/{itemId}`, **then** the API returns the aggregate rating stats: `count` (number of starred ratings), `averageStars` (rounded to 2 decimal places, or null when no ratings), and the `itemId`. No authentication is required.
- **AC 2.4.2** [Authenticated] **Given** I am authenticated, **when** I call `GET /ratings/{itemId}/me`, **then** the API returns my current rating for the item (`id`, `itemId`, `stars`, `createdAt`, `updatedAt`), or a null body if I have not yet rated it.
- **AC 2.4.3** [Anonymous] **Given** I am not authenticated, **when** I call `GET /ratings/{itemId}/me`, **then** the API returns 401.
- **AC 2.4.4** [Authenticated] **Given** I am authenticated, **when** I call `POST /ratings/{itemId}` with `{ "stars": 1..5 }`, **then** the API creates or updates my rating for that item (at most one rating per user per item) and returns 200 with the rating record; repeating the same request updates the stored value.
- **AC 2.4.5** [Authenticated] **Given** I call `POST /ratings/{itemId}` with `{ "stars": null }`, **then** the API stores a null-stars rating (effectively clearing my vote) but keeps the row; the null stars entry is excluded from `count` and `averageStars`.
- **AC 2.4.6** [Anonymous] **Given** I am not authenticated, **when** I call `POST /ratings/{itemId}`, **then** the API returns 401.
- **AC 2.4.7** [Authenticated] **Given** I call `POST /ratings/{itemId}` with a `stars` value outside 1–5, **then** the API returns 400 Bad Request.

**UI**

- **AC 2.4.8** [Anyone] **Given** I am viewing an item detail page, **when** the page loads, **then** I see aggregate rating stats (count, average) from `GET /ratings/{itemId}`.
- **AC 2.4.9** [Authenticated] **Given** I am authenticated and viewing an item, **when** the rating component loads, **then** my existing rating is pre-filled (from `GET /ratings/{itemId}/me`); I can click a star (1–5) to create or update my rating via `POST /ratings/{itemId}`; the displayed stats update after the mutation succeeds.
- **AC 2.4.10** [Anonymous] **Given** I am not authenticated and viewing an item, **when** I see the rating display, **then** ratings are shown read-only; no rate control is offered.

---

### AC 2.5 Item comments

**API**

- **AC 2.5.1** [Anyone] **Given** I call `GET /comments?itemId={guid}`, **then** the API returns all comments for that item ordered by creation date (newest first) including `id`, `itemId`, `text`, `createdBy` (user id), `createdByName` (display name), `createdAt`, and `updatedAt`; no authentication is required.
- **AC 2.5.2** [Authenticated] **Given** I am authenticated, **when** I call `POST /comments` with `{ "itemId": guid, "text": "..." }`, **then** the API creates a comment attached to the item and returns 201 with the created comment; `text` must be non-empty and at most 2000 characters; the item must exist (404 if not found).
- **AC 2.5.3** [Anonymous] **Given** I am not authenticated, **when** I call `POST /comments`, **then** the API returns 401.
- **AC 2.5.4** [Owner of comment] **Given** I am authenticated and I own the comment, **when** I call `PUT /comments/{id}` with `{ "text": "..." }`, **then** the API updates the comment text and sets `updatedAt`, returning 200; `text` must be non-empty and at most 2000 characters.
- **AC 2.5.5** [Authenticated (non-owner)] **Given** I am authenticated but do not own the comment, **when** I call `PUT /comments/{id}`, **then** the API returns 403 Forbidden.
- **AC 2.5.6** [Anonymous] **Given** I am not authenticated, **when** I call `PUT /comments/{id}`, **then** the API returns 401.
- **AC 2.5.7** [Owner of comment] **Given** I am authenticated and I own the comment, **when** I call `DELETE /comments/{id}`, **then** the API deletes the comment and returns 204 No Content.
- **AC 2.5.8** [Authenticated (non-owner)] **Given** I am authenticated but do not own the comment, **when** I call `DELETE /comments/{id}`, **then** the API returns 403 Forbidden.
- **AC 2.5.9** [Anonymous] **Given** I am not authenticated, **when** I call `DELETE /comments/{id}`, **then** the API returns 401.
- **AC 2.5.10** [Anyone] **Given** the comment ID does not exist, **when** I call `PUT` or `DELETE /comments/{id}`, **then** the API returns 404.

**UI**

- **AC 2.5.11** [Anyone] **Given** I am viewing an item detail page, **when** the page loads, **then** I can see the comment count for that item and access a comments section (e.g. inline or via navigation to `/items/{id}/comments`).
- **AC 2.5.12** [Authenticated] **Given** I am authenticated and in a study flow (Flashcards or Quiz), **when** I open comments for the current item, **then** the UI opens a **comments drawer** over the current view without navigating away; the drawer shows existing comments, allows me to add a new comment, and lets me edit or delete my own comments; closing the drawer returns me to my previous position in the study flow.
- **AC 2.5.13** [Owner of comment] **Given** I own a comment, **when** I view it in the comments section or drawer, **then** I see edit and delete controls for that comment; editing opens an inline editor; deleting removes the comment from the list immediately.
- **AC 2.5.14** [Anonymous] **Given** I am not authenticated, **when** I view the comments section, **then** I can read existing comments but the add-comment control is not offered (or is disabled with a sign-in prompt).

---

### AC 2.6 Upload items to collection

This flow lets authenticated users create items in bulk by pasting AI-generated JSON and automatically organizes them into a new private collection.

**API**

- **AC 2.6.1** [Authenticated] **Given** I am authenticated, **when** I call `POST /items/upload-to-collection` with `category` (required), `keyword1` (rank-1, required), `keyword2` (rank-2, required), `keywords` (optional extra keywords list), `items` (array, at least one, max 100 for regular users or 1000 for admins), and `inputText` (raw upload content used for duplicate detection), **then** the API creates the items as private (`IsPrivate = true` for non-admins), creates a new collection with an auto-generated GUID name, adds all successfully created items to that collection, and returns 201 with `collectionId`, `name`, and `itemCount`; validation errors return 400.
- **AC 2.6.2** [Authenticated] **Given** the SHA-256 hash of `inputText` matches an existing upload record for my user, **when** I call `POST /items/upload-to-collection`, **then** the API returns a conflict error (duplicate upload detection); the same JSON content cannot be uploaded twice by the same user.
- **AC 2.6.3** [Anonymous] **Given** I am not authenticated, **when** I call `POST /items/upload-to-collection`, **then** the API returns 401.
- **AC 2.6.4** [Admin] **Given** I am an admin, **when** I call `POST /items/upload-to-collection`, **then** items are created as public (`IsPrivate = false`) instead of private.

**UI**

- **AC 2.6.5** [Authenticated] **Given** I am on the Upload to Collection page at `/items/upload-to-collection`, **when** the page loads, **then** I see: (1) category, rank-1, and rank-2 dropdowns pre-filled from URL params where available; (2) an example AI prompt I can copy and paste into an AI assistant; (3) a text area to paste the AI JSON response or a file upload control to load a JSON file; and (4) a content-compliance warning (see AC 4.7.10).
- **AC 2.6.6** [Authenticated] **Given** I paste valid JSON and select category/rank-1/rank-2, **when** I submit, **then** the UI calls `POST /items/upload-to-collection`; on success it navigates me to the new collection in Flashcards mode (`/explore/collections/{id}`).
- **AC 2.6.7** [Authenticated] **Given** I submit a duplicate upload (same JSON already uploaded before), **when** the API returns a conflict, **then** the UI shows an error explaining that this content was already uploaded.

---

### AC 2.7 Admin: set item visibility

**API**

- **AC 2.7.1** [Admin] **Given** I am an admin, **when** I call `PUT /items/{id}/visibility` with `{ "isPrivate": true|false }`, **then** the API updates the item's `IsPrivate` flag and returns 200 with `id` and `isPrivate`; this allows admins to promote or demote any item between private and public without going through the review board flow.
- **AC 2.7.2** [Authenticated (non-admin)] **Given** I am authenticated but not an admin, **when** I call `PUT /items/{id}/visibility`, **then** the API returns 403 Forbidden.
- **AC 2.7.3** [Anonymous] **Given** I am not authenticated, **when** I call `PUT /items/{id}/visibility`, **then** the API returns 401.
- **AC 2.7.4** [Admin] **Given** the item does not exist, **when** I call `PUT /items/{id}/visibility`, **then** the API returns 404 Not Found.

---

### AC 2.8 Admin review board for items

**API**

- **AC 2.8.1** [Admin] **Given** I am an admin, **when** I call `GET /admin/items/review-board`, **then** the API returns only items where the internal flag `ReadyForReview = true`, ordered by creation date (newest first), including question, answers, category, source, factual risk (if any), and review comments (if any).
- **AC 2.8.2** [Admin] **Given** I am an admin, **when** I call `PUT /admin/items/{id}/approval` for an item that is ready for review, **then** the API makes the item **public** (`IsPrivate = false`) and clears the review flag (`ReadyForReview = false`) so it no longer appears in the review board.
...
- **AC 3.0.5** [Anyone] **Given** I am on the home page, **when** the category section loads, **then** I see a horizontal scrollable carousel of category boxes with image artwork; the categories are bundled with the frontend and do not depend on the database; the first four visible on a full-width desktop are Exams, Trivia, History, Geography (in that order); remaining categories are reachable by scrolling; clicking a box opens `/categories/{categorySlug}`; a **Show all** link navigates to `/categories`. On a medium viewport 3 categories are visible; on a small viewport 2 are visible.
- **AC 3.0.6** [Anyone] **Given** I am on the home page below the categories section, **when** the Featured Sets lane loads, **then** I see a horizontal carousel of featured keyword sets fetched live from `GET /featured` (the same list managed by admins); each card links directly to the category/keyword URL for that set; the lane is hidden when the API returns no sets; a **Show all** link navigates to `/featured`. On a fresh local environment, the startup seeder pre-populates four sets (AWS SAA-C03, World Cup Starter, Tropical Island Survival, Solar System Sprint) so the lane is visible without manual admin setup.
- **AC 3.0.6a** [Anyone] **Given** I am on the home page below the Featured Sets lane, **when** the page loads, **then** I see a **Recently Added Collections** lane showing the 6 most recently created public collections fetched live from `GET /collections/discover` (ordered by creation date descending); each card shows the collection name, description (if any), and item count and links to the collection page; a **Browse all** link navigates to `/collections`; the lane is hidden when no results are returned.
- **AC 3.0.7** [Anyone] **Given** I am anywhere in the SPA, **when** the shared page chrome is visible, **then** I see a footer with **Feedback**, **Ideas**, **About**, **Privacy**, **Terms**, and **User Guide** actions; **Ideas** opens `/ideas`, **About** opens `/about`, and **User Guide** opens the published GitHub Pages guide site that contains both desktop and mobile walkthroughs with an in-page mode switcher.
- **AC 3.0.7a** [Anyone] **Given** I open `/about`, **when** the page loads, **then** I see the current product version and build identifier for the deployed frontend so the running release can be identified without inspecting source or deployment logs.
...
- **AC 3.0.11** [Anyone] **Given** I open `/categories`, `/collections`, `/items/add`, or `/ideas`, **when** the page renders, **then** it uses the dark blue theme (dark background, light text, dark card containers with subtle borders) consistent with the Home page and no white wrapper is applied by the Layout for these routes.
- **AC 3.0.12** [Anyone] **Given** I hover over a category card on the `/categories` overview page, **when** the card has a full description, **then** a native browser tooltip shows the full description text.
- **AC 3.0.13** [Anyone] **Given** I view an item detail page and the **Source** field value starts with `http://` or `https://`, **when** the source is displayed, **then** it renders as a clickable link that opens in a new tab; if the source is not a URL it renders as plain text.
- **AC 3.0.14** [Anyone] **Given** I am on any page on a mobile viewport, **when** the page renders, **then**: (a) the global nav bar is ~56px tall; (b) the footer uses a 3-column button grid on mobile and collapses to a single flex row on sm+; (c) the mode-switcher tabs (Sets / List / Flashcards / Quiz) are ~40px tall with a subtle border-style selected state; (d) the inline filter block is replaced by a compact "Filters" button (with an active-count badge when filters are applied) and a Sort dropdown — clicking the Filters button opens a bottom-sheet modal containing the full filter controls; on category list and sets screens, the mobile **Add** action sits in the same control row between **Filters** and **Map** while the desktop Add action remains in the mode toolbar; (e) scope breadcrumbs use `text-xs` and scroll horizontally if they overflow; (f) the bucket card grid defaults to 2 columns with 8px gaps; (g) compact bucket cards have a minimum height of ~68px with 10px internal padding; (h) item-list cards keep item-detail and collection controls inside the card bounds, aligned beside the category / visibility metadata instead of rendering as an external side rail; (i) the active collection label in those mobile list-card controls may be abbreviated to a compact 3-character token (for example `Sec`), while the full collection name remains available via the manage-collections control.
...
- **AC 3.4.4** [Anyone] **Given** I am on the Sets view and have reached the end of the sets hierarchy (no more buckets; message e.g. "You've reached the end of the sets hierarchy"), **when** the page is shown, **then** leaf scopes follow **AC 3.4.4a**; otherwise the scope secondary bar still shows **Sets | List | Flashcards | Quiz** with **Sets** selected.
- **AC 3.4.4a** [Anyone] **Given** I open a leaf category scope that has no child buckets (for example a category plus rank-2 keyword such as `/categories/exams/act/math`, or any scope whose next-level buckets are empty), **when** the page loads, **then** the UI defaults to **List** for that scope instead of showing an empty Sets screen, and the scope secondary bar omits **Sets** and shows only **List | Flashcards | Quiz**.
...
- **AC 3.5.6** [Anyone] **Given** I open an item's details screen from category/global **List** mode, **when** I use the back control on the item-details screen, **then** I return to the same list URL I came from, including the current category path and active query-string filters.
...
- **AC 3.6.2** [Anyone] **Given** I am on the category Sets or **List** view, **when** I switch to **Flashcards** or **Quiz**, **then** I am taken to Flashcards or Quiz for the **current category/keyword scope** (e.g. `/explore/{categorySlug}` or `/explore/{categorySlug}/item/{itemId}` with same scope); the set of items is the same as in **List** view for that scope. Behavior (one item at a time, navigation, scoring) is the same as for collection-based Flashcards/Quiz where applicable.
- **AC 3.6.2a** [Anyone] **Given** I am in **Flashcards** mode for a **category/keywords** or **global** scope, **when** I view the current item, **then** the **question** is shown face-up on the card; **when** I click the card, **then** it transitions to show the **correct answer** and **explanation** (if any); **when** I click the answer section, **then** I can flip back to the question; **when** I go to the previous or next item, **then** the card resets to show the question face-up. If the explanation exceeds ~220 characters it is initially truncated with a **Show more** toggle that expands the explanation in place without flipping the card; clicking **Show less** collapses it again.
- **AC 3.6.3** [Anyone] **Given** I am on **Flashcards** or **Quiz** for a **category** scope, **when** I view the scope secondary bar, **then** non-leaf scopes show **Sets | List | Flashcards | Quiz** (same as on Sets/**List**), and clicking **Sets** navigates back to the Sets view for that scope. Leaf scopes follow **AC 3.6.3a**. For **collection** scope, the bar shows only **List | Flashcards | Quiz** (no Sets).
- **AC 3.6.3a** [Anyone] **Given** I am on **Flashcards** or **Quiz** for a **leaf category** scope, **when** I view the scope secondary bar, **then** it omits **Sets** and shows only **List | Flashcards | Quiz**.
- **AC 3.6.4** [Anyone] **Given** I am on **Flashcards** or **Quiz** for a category scope, **when** the page is shown, **then** I see the same breadcrumb and item count at the top as in **List** mode (for example `Categories > exams > act > math (3)`), and the mode guidance is shown as a compact hint instead of a large page title plus descriptive paragraph.
- **AC 3.6.5** [Anyone] **Given** I am in **Quiz** mode for a **category or global** scope (not a collection) and I change **Quiz size**, **when** the new size is applied (on leaving the Quiz size field), **then** the app requests a **new random set** of that many items for the current scope, clears in-session cached item lists used for quiz navigation, resets quiz progress for that run, and updates the URL so it no longer points at an item from the previous set unless that item is re-selected by the new load.
- **AC 3.6.5a** [Anyone] **Given** I am in **Quiz** mode, **when** the quiz card is visible, **then** the **Quiz size** input, **item counter** (`X of Z`), and **item-details eye icon** all appear on a single compact stripe above the question, in the layout `{Quiz size} | {< X of Z >} | {👁}`, so that the question, answer options, score, and the Previous/Next buttons are all visible on a standard laptop screen at 100% zoom without scrolling.
...
On the Categories page (Sets view and List view), a **filter dialog** lets the user change the category/keyword scope and apply **scope filters** (item type, search, rating). Filters are reflected in the URL; applying them navigates and updates the **scope**. The dialog is opened by a **Filters** button and closes via **Show results** or the close (×) button. On all screen sizes it appears as a centered modal (not a bottom sheet).

**Filter dialog (open/close and active state)**

- **AC 3.9.1** [Anyone] **Given** I am on the Categories page in Sets or List view, **when** the page loads, **then** I see a **Filters** button in the control bar; clicking it opens the filter dialog as a centered modal. The dialog closes when I click **Show results**, press Escape, or click the backdrop. When I click **Show results** after applying changes, the dialog closes.
- **AC 3.9.2** [Anyone] **Given** I am on the Categories page, **when** any filter is in effect (e.g. I am not on "All categories", or I have path/query keywords, or I have scope filters such as item type, search, or rating), **then** the Filters button shows an **Active** indicator (badge). Inside the dialog a **Clear all** link resets all filter controls to the default or current-scope state.

**Keyword tag filters (Sets and List view)**

- **AC 3.9.2a** [Anyone] **Given** the filter dialog is open in **List** view, **then** I see a **Keywords** section at the top with a search input. As I type, a dropdown suggests up to **5** matching keyword names from the currently loaded items (in-memory match, no network call). Selecting a suggestion adds that keyword to the active query filters immediately (the URL updates); the input resets so I can add more keywords. Active keyword filters are shown as removable chips beneath the input. Pressing **Enter** in the input field adds the typed text as a keyword filter when it exactly matches a suggestion; if no suggestions exist, pressing Enter adds the free-text value as a filter.
- **AC 3.9.2b** [Anyone] **Given** the filter dialog is open in **Sets** view for a specific category, **then** I see a **Keywords** section at the top with a search input. As I type, a dropdown suggests up to **5** matching item-level tag keyword names fetched from the API for that category (cached per category, no call for "All categories"). Selecting a suggestion, or pressing **Enter** to submit free text, adds that keyword to `?keywords=` in the URL immediately while keeping the Sets view (`?view=sets`); the input resets so I can add more. Active keyword filters are shown as removable chips beneath the input; clicking a chip removes that keyword from the URL.

**Category and keyword filters (navigation scope)**

- **AC 3.9.3** [Anyone] **Given** the filter dialog is visible, **when** I use the filters, **then** I see: (1) a **Category** dropdown (e.g. "All categories" or a specific category), (2) an optional **Primary topic** (rank-1 keyword) control, and (3) an optional **Subtopic** (rank-2 keyword) control, disabled until a primary topic is selected. I can click **Apply** to navigate to the selected category/keyword scope; the URL path and optional query keywords are updated, page is reset to 1, and the dialog closes.
- **AC 3.9.4** [Anyone] **Given** I have selected "All categories" in the filter dialog and optionally one or more keywords, **when** I click Apply, **then** the scope becomes all categories with those keywords as query filters (e.g. slug `all` or `/categories` with `?keywords=...`); the path does not include category/keyword segments.
...
- **AC 3.10.1a** [Anyone] **Given** I browse category navigation in the SPA, **when** taxonomy-backed pages load, **then** they use `GET /taxonomy` as the canonical tree of category to primary-topic (L1) to subtopic (L2) paths for valid `/categories/...` navigation scopes.
...
- **AC 4.7.7** [Authenticated] **Given** I am authenticated, **when** I open my profile, **then** I can view and change my default pagination (PageSize) via a dropdown with options 5, 10, 25, 50, 100; selecting a value persists it immediately via `PUT /users/settings` with no extra save step required.
- **AC 4.7.8** [Authenticated] **Given** I am on a page that uses default pagination (e.g. Categories items list, Collection detail), **when** the URL does not specify `pagesize`, **then** my saved PageSize setting is used; if the URL specifies `pagesize`, the URL value takes precedence for that page/session.
...
- **AC 4.7.11** [Anyone] **Given** I open the Categories items list or Collection detail list, **when** the list loads, **then** answers are shown by default (answer text visible on item cards with green highlight).
- **AC 4.7.12** [Anyone] **Given** I am in a list view (Categories items list or Collection detail), **when** the list is visible, **then** a compact **Show answers / Hide answers** toggle button appears in the control bar between the Filters button and the Map/Sort controls (Categories) or at the top of the item list (Collection detail). The toolbar renders as soon as the collection has at least one item, regardless of current page. On mobile only an eye icon is shown; on desktop the label "Show answers" or "Hide answers" is also displayed.
- **AC 4.7.13** [Anyone] **Given** answers are shown, **when** item cards render, **then** every answer text is shown with a light green background (`bg-green-100`) matching the correct-answer highlight style used in quiz mode.
...
- **AC 4.7.14a** [Anyone] **Given** a list URL contains `?showAnswers=true` or `?showAnswers=false`, **when** the page loads, **then** the answer visibility is initialised from that parameter, overriding the user's stored preference for that visit. After mount the toggle continues to function normally and saves preferences as usual.
- **AC 4.7.15** [Authenticated] **Given** I am authenticated, **when** I toggle the answer visibility in a list view, **then** the setting is persisted as `HideAnswers` via `PUT /users/settings`; value `"true"` means hide, `"false"` means show. The default value (when the setting is absent) is `"false"` (shown).
- **AC 4.7.16** [Anonymous] **Given** I am not authenticated, **when** I toggle the answer visibility, **then** the preference is stored in `sessionStorage` only and reverts to shown on a new browser session.
...
The ideas board is a public-facing product planning surface at `/ideas`. It shows published ideas grouped by lifecycle status and supports moderation for new submissions.

**API**

- **AC 4.9.1** [Anyone] **Given** I call `GET /ideas`, **when** the request succeeds, **then** the API returns only ideas whose moderation state is `Published`, and each idea includes the user-facing board fields: `title`, `problem`, `proposedChange`, optional `tradeOffs`, `status`, author/review metadata, rating summary, comment count, and permission booleans used by the SPA.
- **AC 4.9.2** [Authenticated] **Given** I call `GET /ideas/mine`, **when** the request succeeds, **then** the API returns only ideas I created across `PendingReview`, `Published`, and `Rejected` moderation states so I can track my private submissions separately from the public board.
- **AC 4.9.3** [Authenticated] **Given** I call `POST /ideas` with `title`, `problem`, `proposedChange`, optional `tradeOffs`, and a valid Cloudflare Turnstile token, **when** the payload passes validation, **then** the API creates a new idea with `status = Proposed` and `moderationState = PendingReview`; the new idea is visible in my private submissions immediately but is not returned by `GET /ideas` until approved.
- **AC 4.9.4** [Authenticated] **Given** I submit an idea that is an exact normalized duplicate of one of my existing ideas, exceeds the daily quota, contains blocked profanity, or fails Turnstile verification, **when** the request is processed, **then** the API rejects it with an error (`409` for duplicate/quota conflicts, `400` for blocked content or verification/validation failures, `429` when the dedicated create rate limit is exceeded) and does not create a public idea row.
- **AC 4.9.5** [Owner/Admin] **Given** I call `PUT /ideas/{id}` for an idea I am allowed to edit, **when** the payload is valid, **then** the API updates the content fields, normalizes text, re-checks profanity/duplicates, and if a non-admin submitter edits a `Published` or `Rejected` idea the moderation state is moved back to `PendingReview` for re-review.
- **AC 4.9.6** [Owner/Admin] **Given** I call `DELETE /ideas/{id}` for an idea I am allowed to delete, **when** the request succeeds, **then** the API deletes the idea and its dependent idea-comments and idea-ratings.
- **AC 4.9.7** [Admin] **Given** I call `PUT /ideas/{id}/status`, **when** the payload contains one of `Proposed`, `Planned`, `In Progress`, `Shipped`, or `Archived`, **then** the API updates only the public lifecycle status of that idea.
- **AC 4.9.8** [Anyone/Authenticated] **Given** an idea is `Published`, **when** I call `GET /ideas/{id}/comments`, **then** comments are readable publicly; **when** an authenticated user calls `POST /ideas/{id}/comments`, `PUT /ideas/{id}/comments/{commentId}`, or `DELETE /ideas/{id}/comments/{commentId}`, **then** create is allowed only on published ideas, comment create/update pass the moderation filter, comment create is rate-limited, and only the comment owner may edit or delete that comment.
- **AC 4.9.9** [Authenticated] **Given** an idea is `Published`, **when** I call `POST /ideas/{id}/rating` with `stars` from 1 to 5 or `null`, **then** the API upserts exactly one rating row per `(idea, user)` pair; unpublished ideas reject rating requests.
- **AC 4.9.10** [Admin] **Given** I call `GET /admin/ideas?moderationState={state}`, `POST /admin/ideas/{id}/approve`, or `POST /admin/ideas/{id}/reject`, **when** the request is valid, **then** the admin APIs list ideas by moderation state and allow approval to `Published` or rejection to `Rejected`; rejection requires `moderationNotes`, and moderation actions are recorded in audit logs.
- **AC 4.9.11** [System] **Given** the application starts after migrations, **when** bootstrap seeding runs, **then** a checked-in SQL script inserts curated idea rows idempotently (missing seeded ideas are added, existing edited ideas are left alone, and user-created ideas are never deleted).

**UI**

- **AC 4.9.12** [Anyone] **Given** I open `/ideas`, **when** the page loads, **then** I see a public board grouped in this fixed order: `Proposed`, `Planned`, `In Progress`, `Shipped`, `Archived`; the board is searchable across the visible idea text and supports `Recently updated` and `Top rated` sorting.
- **AC 4.9.13** [Anyone] **Given** I view an idea card on `/ideas`, **when** it is collapsed, **then** I see a compact summary with the title, problem snippet, proposed-change snippet, optional trade-offs snippet, status, rating summary, and comment count; **when** I expand it, **then** I see the full fields plus discussion.
- **AC 4.9.14** [Anonymous] **Given** I browse `/ideas` without signing in, **when** I view published ideas, **then** I can read ideas and comments but I am prompted to sign in before I can submit an idea, rate, or post a comment.
- **AC 4.9.15** [Authenticated] **Given** I am signed in on `/ideas`, **when** I click **Share an idea**, **then** I see a modal with `Title`, `Problem`, `Proposed change`, optional `Trade-offs`, and Turnstile verification; submitting the form sends the idea into moderation rather than publishing it immediately.
- **AC 4.9.16** [Authenticated] **Given** I am signed in on `/ideas`, **when** I view **My submissions**, **then** I see my own ideas grouped by `Pending review`, `Published`, and `Rejected`, including moderation notes for rejected or flagged ideas; unpublished ideas in this panel do not expose public comments/ratings.
- **AC 4.9.17** [Admin] **Given** I open `/admin/ideas`, **when** the page loads, **then** I see moderation tabs for `Pending review`, `Published`, and `Rejected`, plus controls to approve, reject with a note, edit, delete, and update lifecycle status on each idea.
- **AC 4.9.18** [Anyone] **Given** a seeded idea is shown in the ideas UI, **when** its stored author is the internal seeder identity, **then** the displayed author name is `Quizymode` rather than the raw seeding account value.
...
### AC 5.2 Admin: repo-managed seed sync for items and public collections

**Design:** Admin seed sync reads canonical repo files directly from GitHub at a specific ref instead of accepting a pasted/uploaded manifest. The runtime request supplies repository owner/name, git ref, and the API fetches the pre-built items bundle plus public collection files from GitHub. It validates the resulting item and collection set, then previews or applies idempotent upserts by permanent `itemId` (`Items.Id`) and `collectionId` (`Collections.Id`). Categories and navigation still use taxonomy slugs (`category`, `navigationKeyword1`, `navigationKeyword2`), not database keyword IDs. Missing items or collections are **not** deleted implicitly; deleting or consolidating repo-managed content remains an explicit maintenance action.

**API**

- **AC 5.2.1** [Admin] **Given** I am an admin, **when** I call `POST /admin/seed-sync/preview` with a JSON body containing `schemaVersion`, `repositoryOwner`, `repositoryName`, `gitRef`, and optional `deltaPreviewLimit`, **then** the API fetches the canonical items bundle and public collection seed files from GitHub, validates the loaded item and collection set, and returns 200 with source metadata (repo/ref/resolved commit/items path/collections path), item summary counts, collection summary counts, and a `changes` array containing only delta items (not unchanged items), capped by `deltaPreviewLimit`.
- **AC 5.2.2** [Admin] **Given** I call preview or apply with invalid input (for example duplicate `itemId` or `collectionId` values in the loaded source, an unsupported `schemaVersion`, an invalid taxonomy category, an invalid navigation path, an `itemId` that already belongs to a private or non-repo-managed row, or a collection that references a missing item), **when** the request is processed, **then** the API returns 400 Bad Request with a validation or contract error and does not mutate data.
- **AC 5.2.3** [Admin] **Given** I call `POST /admin/seed-sync/apply` with an item whose `itemId` already exists on a public repo-managed row, **when** the source-controlled content differs, **then** the API **updates** that existing DB row in place (question, answers, explanation, source, navigation, and extra keywords) instead of creating a duplicate item. **Contrast with user bulk-create:** `POST /items/bulk` always inserts new rows and never persists a caller-supplied repo identity.
...
- **AC 5.2.6a** [Admin] **Given** the GitHub-loaded public collection set contains a `collectionId` that already exists on a repo-managed public collection, **when** the source-controlled name, description, or item membership differs, **then** `POST /admin/seed-sync/apply` updates that existing collection in place instead of creating a duplicate.
- **AC 5.2.6b** [Admin] **Given** the GitHub-loaded public collection set contains a `collectionId` that does **not** exist in the current database, **when** the request succeeds, **then** the API creates a new public repo-managed collection using that explicit `collectionId`, links the referenced items, and marks the row with `IsRepoManaged = true`.
- **AC 5.2.6c** [Admin] **Given** some existing public repo-managed collections are absent from the GitHub-loaded collection set for the requested ref, **when** I call preview or apply, **then** the API leaves those collection rows untouched; current apply behavior does **not** infer deletes from missing collections.
...
- **AC 5.2.9** [Admin] **Given** an admin UI is built on top of the seed-sync API, **when** it requests a preview, **then** it should show the returned delta rows and summary counts rather than trying to render the full GitHub-loaded item set.
- **AC 5.2.10** [Admin] **Given** I am on the Seed Sync screen, **when** I enter a GitHub repository and git ref, **then** the UI sends that repo/ref request to the API instead of uploading a manifest JSON payload; the screen shows both item and public-collection sync counts from the API response.
- **AC 5.2.11** [Admin] **Given** canonical seed-source files in GitHub contain extra fields that are not part of the seed-sync item schema (such as `isPrivate`, `factualRisk`, `reviewComments`, or editor-only arrays like `factChecks` with optional nested fields such as `source` URLs), **when** the API loads those files, **then** those extra fields are silently ignored; repo-managed items are always public (`IsPrivate = false`) regardless of any `isPrivate` value in source. **Note:** Some repo-managed exam items may append an `UPD260411:` line to the human-readable `explanation` string (plain text) to record a documentation excerpt and URL; that suffix is persisted like any other explanation text. Editorial passes may also rewrite the prose before that suffix (for example replacing generic filler or correcting a mismatched keyed answer) while keeping `itemId` stable for seed sync.
...
- **AC 5.2.15** [Admin/System] **Given** the item registry is generated by `scripts/build_item_registry.py`, **when** the script runs, **then** it produces `data/seed-source/_registry/source-file-index.json` — a flat index of all 866+ source files with `path`, `category`, `nav1`, `nav2`, `itemCount`, and `modifiedAt` (last git commit date); no duplicate shard files or bundle are generated; the original `items/` source files remain the single source of truth.
- **AC 5.2.16** [Admin] **Given** I call `POST /admin/seed-sync/preview` or `POST /admin/seed-sync/apply` with a `sinceCommitSha`, **when** the backend resolves the current commit, **then** it fetches `source-file-index.json` (~212 KB), calls the GitHub compare API to identify changed source file paths, downloads only those original source files from `data/seed-source/items/`, and processes only the items they contain; items already in the DB are not re-examined.
- **AC 5.2.17** [Admin] **Given** I call the apply endpoint without `sinceCommitSha` (full sync), **when** the request includes `fileOffset` and `fileBatchSize`, **then** the backend processes the slice `files[fileOffset : fileOffset + fileBatchSize]` of the source-file index, returns `isComplete = false` with `nextFileOffset` when more files remain, and returns `isComplete = true` on the final batch; the frontend loops until complete.
- **AC 5.2.17a** [Admin/System] **Given** repo-managed item source JSON is maintained under `data/seed-source/items`, **when** files are created, merged, renamed, or chunked, **then** each file name follows `<category>.<nav1>.<nav2>.json` for sets with 100 or fewer items, or `<category>.<nav1>.<nav2>.<chunk>.json` with zero-padded chunk suffixes (`001`, `002`, ...) for larger sets split into chunks of at most 100 items. Files must not use ad hoc small numeric suffixes (for example `.2.json`) when the set can fit in one semantic file. Moving or chunking items must preserve the existing `itemId` for the same logical repo-managed item.
- **AC 5.2.18** [Admin] **Given** the Seed Sync UI has sync history, **when** the page loads, **then** an "Incremental sync" toggle is shown pre-checked, with the last successful commit SHA pre-filled as `sinceCommitSha`; when the toggle is off, the UI displays a full-sync warning and a progress bar showing files processed during batched apply.
- **AC 5.2.19** [Admin/System] **Given** `ApplyManifestAsync` is called with any number of items, **when** it processes changes, **then** it flushes `SaveChangesAsync` every 200 items within the open transaction rather than accumulating all entity changes and saving once; this prevents memory and timeout issues at scale.
- **AC 5.2.20** [System/SEO] **Given** `scripts/build_item_registry.py` runs in CI, **when** it completes, **then** it also regenerates `src/Quizymode.Web/public/sitemap.xml` with three tiers of category URLs derived from `source-file-index.json`: (a) `/categories/{cat}` at priority 0.8 with `lastmod` = max of all L2 dates under that category, (b) `/categories/{cat}/{nav1}` at priority 0.7 with `lastmod` = max of all L2 dates under that L1, (c) `/categories/{cat}/{nav1}/{nav2}` at priority 0.6 with `lastmod` from the source file's last git commit date; static app pages are preserved with their fixed dates.
...
- **AC 6.3.1** [Authenticated] **Given** I own an import session with generated chunks, **when** I call `POST /study-guides/import/sessions/{id}/chunks/{chunkIndex}/result` with `{ "rawResponseText": "<AI text>" }`, **then** the API attempts to parse `rawResponseText` as JSON, requires a top-level array of objects, runs item-level validation, and — when valid — stores an **enriched** JSON array (not raw AI text) as `parsedItemsJson`; enrichment overrides `category`, `navigationKeyword1`, and `navigationKeyword2` from session values (preventing AI hallucination or tampering), silently truncates `source` to 200 chars instead of rejecting, and does **not** add or persist repo-managed identity fields such as `seedId` or `itemId`. Extra fields in the AI response (such as `isPrivate`, `factualRisk`, or `reviewComments`) are silently ignored. The endpoint returns 200 with `validationStatus` (`Valid` or `Invalid`), validation messages, and the enriched JSON string when valid.
- **AC 6.3.1a** [System] The AI prompts for both study guide import and the standalone bulk-create flow include explicit field length limits in the schema hint: `question` ≤1000 chars, `correctAnswer` and each `incorrectAnswers` item ≤500 chars, `source` ≤200 chars (URL only), `explanation` ≤4000 chars.
...
## AC 7. Developer tooling

### AC 7.1 Automated user guide screenshots

- **AC 7.1.0** [Dev tooling] User-guide screenshots are captured from either production (`https://www.quizymode.com`) or local (`http://localhost:7000`) using the existing Playwright screenshot projects for desktop and mobile. Production publishing does not commit generated screenshots back into the repo; GitHub Actions stores them in temporary build folders, assembles a static Pages site artifact, and deploys only the latest published guide.
- **AC 7.1.1** [Dev tooling] Running `npx playwright test --project=screenshots` authenticates as the test user (credentials from `TEST_USER_EMAIL` / `TEST_USER_PASSWORD` env vars), visits each major signed-in route, and saves a full-page PNG screenshot to `docs/user-guide/screenshots/user/{slug}.png`. Running `npx playwright test --project=screenshots-mobile` captures the same slug inventory to `docs/user-guide/screenshots/mobile/{slug}.png` using a phone-sized browser profile. The target URL is controlled by `PLAYWRIGHT_BASE_URL` (defaults to `https://www.quizymode.com`).
- **AC 7.1.2** [Dev tooling] Running `node scripts/generate-user-guide.js --guide desktop` reads desktop screenshots and rebuilds the desktop markdown guide; `--guide mobile` rebuilds the mobile markdown guide; `--all` rebuilds both. Running `node scripts/build-user-guide-site.js` assembles one static user-guide site with `/assets/desktop/` and `/assets/mobile/` screenshots, a client-side Desktop or Mobile toggle, automatic initial mode selection from viewport width, and persisted user preference in `localStorage`.
...
### AC 7.2 Local E2E runner scripts

- **AC 7.2.1** [Dev tooling] `scripts/run-e2e-local-smoke.ps1` (Windows) and `scripts/run-e2e-local-smoke.sh` (Mac/Linux/WSL) start the local Aspire host and React dev server, wait for both to become healthy, run Playwright auth setup, then execute only tests tagged `@smoke` via `--project=smoke`.
- **AC 7.2.2** [Dev tooling] `scripts/run-e2e-local-full.ps1` and `scripts/run-e2e-local-full.sh` perform the same startup sequence and then execute the full E2E suite via `--project=e2e-full`.
- **AC 7.2.3** [Dev tooling] Both runner pairs share logic extracted into `scripts/_e2e-common.ps1` and `scripts/_e2e-common.sh` (prefixed `_` to signal they are not runnable directly). Shared functions cover: prerequisite checks, Aspire/React process management, health-poll loops, auth setup invocation, environment teardown (including on Ctrl+C), and a post-run summary that parses `playwright-report/results.json` and prints an AC-ID table.
- **AC 7.2.4** [Dev tooling] All four runner scripts read credentials exclusively from `TEST_USER_EMAIL` and `TEST_USER_PASSWORD` environment variables; no credentials are hardcoded. The `PLAYWRIGHT_BASE_URL` environment variable is set to `http://localhost:7000` before Playwright runs so the config targets the local dev stack instead of production.
- **AC 7.2.5** [Dev tooling] E2E test files live under `playwright/e2e/`. Tests tagged `@smoke` (substring in the test title) are included in the smoke project; all tests in that directory run under `e2e-full`.
- **AC 7.2.6** [Dev tooling] Each local runner script saves Playwright test artifacts (screenshots, traces) to a timestamped subfolder under `playwright/test-results/` named `smoke-YYYY-MM-DD_HH-mm-ss` or `full-YYYY-MM-DD_HH-mm-ss`. The folder path is printed at the end of the run. Subfolders older than 7 days are deleted automatically at the end of each run. The `smoke` and `e2e-full` Playwright projects are configured with `screenshot: "on"` so a screenshot is captured after every test.
- **AC 7.2.7** [Dev tooling] Local Playwright runs fail fast by default: failed tests are **not retried**, the per-test timeout is short (about 15 seconds), and default action / expectation waits are only a few seconds so broken selectors do not consume a minute per failure.

### AC 7.3 E2E test coverage baseline

- **AC 7.3.1** [Dev tooling] `playwright/e2e/auth.spec.ts` covers the login flow (AC 4.3): login page renders the sign-in form; valid credentials redirect away from `/login`; invalid credentials show an error without navigating away. Tests run without pre-loaded auth state so they exercise the full sign-in path.
- **AC 7.3.2** [Dev tooling] `playwright/e2e/categories.spec.ts` covers category browsing (AC 3.1, 3.4): `/categories` shows category cards; clicking a category with items navigates to the scoped URL; the category Sets view shows mode tabs (**List**, **Quiz**) and keyword buckets.
- **AC 7.3.3** [Dev tooling] `playwright/e2e/items.spec.ts` covers item access (AC 2.3, 3.5): the **List** tab shows item links after switching modes; clicking an item opens the detail page with Question and Answer sections; the back control returns to the same list URL.
- **AC 7.3.4** [Dev tooling] `playwright/e2e/collections.spec.ts` covers the collections page (AC 1.1, 1.3, 1.9): page loads with **Mine / Bookmarked / Discover** tabs; authenticated users see a create-collection action; switching to the Discover tab updates the URL query string to `?tab=discover`.
- **AC 7.3.5** [Dev tooling] `playwright/e2e/quiz.spec.ts` covers quiz mode (AC 3.6): quiz loads with a question and answer options after navigating from the category Sets view; selecting an answer reveals the correct answer; the Next button advances to the next question; the score counter is visible throughout.
