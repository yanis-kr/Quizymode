...
- **AC 2.2.6.16** [Authenticated] **Given** the AI response includes a `seedId` field on each item (a UUID generated by the AI), **when** the app parses the response, **then** it extracts and preserves the `seedId` for each item that has a valid UUID-format value; items without a valid UUID `seedId` are accepted without one. When accepted items are submitted to the bulk-create API (`POST /items/bulk`), each item's `seedId` is included in the request. The backend always **inserts** new items — it never updates an existing item by `seedId` in this flow. If an incoming `seedId` already exists in the database **for the same user**, the item is treated as a **duplicate and skipped** (same as a text-based duplicate); this prevents re-importing the same JSON file from creating duplicate items. If an incoming `seedId` exists in the database **for a different user** (cross-user collision), the backend automatically generates a new unique ID for the conflicting item, inserts it as a new row, and returns the list of original IDs that were replaced in `reassignedSeedIds`; the UI shows this as a notice in the result modal after saving. The stored `seedId` on the created item has `IsSeedManaged = false`, distinguishing user-created items from admin-managed seed items. **Contrast with admin seed-sync:** seed-sync with an existing `seedId` performs an in-place **update** of the existing row rather than inserting a new one — the bulk-create flow never overwrites existing content regardless of `seedId`.
...
- **AC 5.2.11** [Admin] **Given** I paste or upload a JSON file into the Seed Sync screen, **when** the root of the JSON is a **plain array** (e.g. a file saved from the AI bulk-create flow), **then** the UI automatically wraps it into a valid manifest by generating a `seedSet` name derived from the first item's `category/navigationKeyword1` (falling back to `import-{date}` if not readable) and setting `schemaVersion: 1`; the resulting manifest is submitted to the API exactly as a manually authored manifest would be. This allows AI-generated JSON files (from bulk-create or study-guide import) to be dropped directly into Seed Sync without manual reformatting.
- **AC 5.2.12** [Admin] **Given** an incoming seed-sync item payload contains extra fields that are not part of the seed-sync item schema (such as `isPrivate`, `factualRisk`, or `reviewComments`), **when** the manifest is processed, **then** those extra fields are silently ignored; seed-managed items are always public (`IsPrivate = false`) regardless of any `isPrivate` value in the payload.
...
- **AC 6.3.1** [Authenticated] **Given** I own an import session with generated chunks, **when** I call `POST /study-guides/import/sessions/{id}/chunks/{chunkIndex}/result` with `{ "rawResponseText": "<AI text>" }`, **then** the API attempts to parse `rawResponseText` as JSON, requires a top-level array of objects, runs item-level validation, and — when valid — stores an **enriched** JSON array (not raw AI text) as `parsedItemsJson`; enrichment overrides `category`, `navigationKeyword1`, `navigationKeyword2` from session values (preventing AI hallucination or tampering), ensures every item has a valid `seedId` UUID (generates one if absent or malformed), and silently truncates `source` to 200 chars instead of rejecting. Extra fields in the AI response (such as `isPrivate`, `factualRisk`, `reviewComments`) are silently ignored. The endpoint returns 200 with `validationStatus` (`Valid` or `Invalid`), validation messages, and the enriched JSON string when valid.
