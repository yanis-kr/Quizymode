...
| **Seed set** | A logical source-controlled manifest label for repo-managed items, such as `core-public-items`. Seed sync requests still include this label for preview/apply context, but per-item identity is the permanent `itemId`. |
| **Seed-managed item** | A public item whose content is controlled by the admin seed-sync process and identified by a permanent repo-managed `itemId` that matches `Items.Id`. The item row itself does not store extra seed metadata columns. |
...
**Design:** Admin seed sync imports a **source-controlled manifest** for one **seed set** at a time. The runtime API contract is one manifest payload per request, built from canonical repo files whose required per-item identity field is `itemId`. `itemId` is permanent and maps directly to `Items.Id`. Categories and navigation use taxonomy slugs (`category`, `navigationKeyword1`, `navigationKeyword2`), not database keyword IDs. Preview returns the create/update delta for the uploaded manifest, and apply upserts repo-managed public items by `itemId`. Missing items are **not** deleted implicitly; deleting or consolidating repo-managed items remains an explicit maintenance action.

**API**

- **AC 5.2.1** [Admin] **Given** I am an admin, **when** I call `POST /admin/seed-sync/preview` with a JSON body containing `schemaVersion`, `seedSet`, `items`, and optional `deltaPreviewLimit`, **then** the API validates the request and returns 200 with summary counts at least for `existingItemCount`, `created`, `updated`, and `unchanged`, plus a `changes` array containing only delta items (not unchanged items), capped by `deltaPreviewLimit`.
- **AC 5.2.2** [Admin] **Given** I call preview or apply with invalid input (for example duplicate `itemId` values in the request, an unsupported `schemaVersion`, an invalid taxonomy category, an invalid navigation path, or an `itemId` that already belongs to a private or non-seeder-owned row), **when** the request is processed, **then** the API returns 400 Bad Request with a validation or contract error and does not mutate data.
- **AC 5.2.3** [Admin] **Given** I call `POST /admin/seed-sync/apply` with an item whose `itemId` already exists on a public seeder-owned row, **when** the source-controlled content differs, **then** the API **updates** that existing DB row in place (question, answers, explanation, source, navigation, and extra keywords) instead of creating a duplicate item. **Contrast with user bulk-create:** `POST /items/bulk` always inserts new rows and never persists a caller-supplied repo identity.
- **AC 5.2.4** [Admin] **Given** I call `POST /admin/seed-sync/apply` with an item whose `itemId` does **not** exist in the current database (for example because the row was manually deleted earlier), **when** the request succeeds, **then** the API creates a new public seeder-owned item using that explicit `itemId`.
...
- **AC 5.2.6** [Admin] **Given** some existing public seeder-owned rows are absent from the uploaded manifest, **when** I call preview or apply, **then** the API leaves those rows untouched; current apply behavior does **not** infer deletes from missing rows.

**UI**

- **AC 5.2.9** [Admin] **Given** an admin UI is built on top of the seed-sync API, **when** it requests a preview, **then** it should show the returned delta rows and summary counts rather than trying to render the full uploaded manifest.
- **AC 5.2.10** [Admin] **Given** I paste or upload a JSON file into the Seed Sync screen, **when** the root of the JSON is a **plain array**, **then** the UI may wrap it into a valid manifest only if each item already includes an explicit `itemId`; raw AI output without permanent `itemId` values must be normalized through the repo import tooling before admin seed sync.
- **AC 5.2.11** [Admin] **Given** an incoming seed-sync item payload contains extra fields that are not part of the seed-sync item schema (such as `isPrivate`, `factualRisk`, or `reviewComments`), **when** the manifest is processed, **then** those extra fields are silently ignored; repo-managed items are always public (`IsPrivate = false`) regardless of any `isPrivate` value in the payload.
...
- **AC 7.1.2** [Dev tooling] Running `node scripts/generate-user-guide.js` reads all PNGs from `docs/user-guide/screenshots/user/` and writes `docs/user-guide/user-guide.md` with a table of contents and feature sections embedding each screenshot.
