...
- **AC 3.0.6a** [Anyone] **Given** I am on the home page below the Featured Sets lane, **when** I view the Featured Collections lane, **then** I see a horizontal carousel of featured public collections; each card links to the collection page; the set of cards (collection IDs, names, labels) is driven by the `featuredCollectionCards` array in `src/features/home/homePageData.ts`; the lane is hidden when the array is empty.
