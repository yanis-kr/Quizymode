...
**Design:** Admin seed sync reads canonical repo files directly from GitHub at a specific ref instead of accepting a pasted/uploaded item manifest. The runtime request supplies repository owner/name, git ref, and an optional items path under `data/seed-source/items`. The API fetches those canonical files from GitHub, validates the resulting item set, and then previews or applies an upsert by permanent `itemId` (`Items.Id`). Categories and navigation still use taxonomy slugs (`category`, `navigationKeyword1`, `navigationKeyword2`), not database keyword IDs. Missing items are **not** deleted implicitly; deleting or consolidating repo-managed items remains an explicit maintenance action. Public collection sync is still a separate seeding/reset concern and is not yet part of the GitHub-backed admin sync endpoint.

**API**

- **AC 5.2.1** [Admin] **Given** I am an admin, **when** I call `POST /admin/seed-sync/preview` with a JSON body containing `schemaVersion`, `repositoryOwner`, `repositoryName`, `gitRef`, optional `itemsPath`, and optional `deltaPreviewLimit`, **then** the API fetches canonical seed-source files from GitHub, validates the loaded item set, and returns 200 with source metadata (repo/ref/resolved commit/items path), summary counts at least for `existingItemCount`, `created`, `updated`, and `unchanged`, plus a `changes` array containing only delta items (not unchanged items), capped by `deltaPreviewLimit`.
- **AC 5.2.2** [Admin] **Given** I call preview or apply with invalid input (for example duplicate `itemId` values in the request, an unsupported `schemaVersion`, an invalid taxonomy category, an invalid navigation path, or an `itemId` that already belongs to a private or non-repo-managed row), **when** the request is processed, **then** the API returns 400 Bad Request with a validation or contract error and does not mutate data.
