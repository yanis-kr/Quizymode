...
- {progress}=.local/seed-progress/{category}/_progress.md
- {state}=.local/seed-progress/{category}/_{category}_state.md
```

The `{progress}` and `{state}` files are local continuity files. They live under `.local/seed-progress/`, which is ignored by git.
