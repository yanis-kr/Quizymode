...
internal static partial class StartupExtensions
{
    public static WebApplicationBuilder AddSwaggerServices(this WebApplicationBuilder builder)
    {
        ...
        builder.Services.AddOpenApi("v1", options =>
        {
            options.AddDocumentTransformer((document, context, cancellationToken) =>
            {
                ...
                // Rename generic schema names to meaningful ones
    ...
    private static void RenameSchemas(OpenApiDocument document)
    {
        ...
        Dictionary<string, string> schemaRenames = new();
        ...
        if (document.Paths is not null)
        {
            foreach (KeyValuePair<string, OpenApiPathItem> path in document.Paths)
            {
                ...
                if (pathItem.Operations is not null)
                {
                    ...
                    {
                        ProcessOperation(operation.Value, pathKey, operation.Key.ToString(), schemaRenames);
                    }
                }
            }
        }
        ...
        // Apply renames
        foreach (KeyValuePair<string, string> rename in schemaRenames)
        {
