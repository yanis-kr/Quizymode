...
export const itemsApi = {
  ...
  getRandom: async (
    ...
    if (category) params.category = category;
    if (subcategory) params.subcategory = subcategory;
    const response = await apiClient.get<RandomItemsResponse>('/items/random', { params });
    return response.data;
  },
  ...
  bulkCreate: async (data: BulkCreateItemsRequest): Promise<{ created: number; skipped: number }> => {
    const response = await apiClient.post<{ created: number; skipped: number }>('/items/bulk', data);
    return response.data;
  },

  setVisibility: async (id: string, isPrivate: boolean): Promise<ItemResponse> => {
    const response = await apiClient.put<ItemResponse>(`/items/${id}/visibility`, { isPrivate });
    return response.data;
  },
};
