...
export const ratingsApi = {
  getStats: async (itemId: string): Promise<RatingStatsResponse> => {
    const response = await apiClient.get<{ stats: RatingStatsResponse }>(`/ratings/${itemId}`);
    return response.data.stats;
  },

  getUserRating: async (itemId: string): Promise<RatingResponse | null> => {
    const response = await apiClient.get<RatingResponse | null>(`/ratings/${itemId}/me`);
    return response.data;
  },

  createOrUpdate: async (data: CreateRatingRequest): Promise<RatingResponse> => {
    const response = await apiClient.post<RatingResponse>(`/ratings/${data.itemId}`, {
      stars: data.stars,
    });
    return response.data;
  },
