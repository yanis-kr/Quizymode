...
import ExploreModePage from "./ExploreModePage";
...
vi.mock("@/components/study/ExploreRenderer", () => ({
  ExploreRenderer: () => <div>Explore Renderer</div>,
}));
...
describe("ExploreModePage", () => {
  ...
  it("shows the leaf scope header and routes List back to the scoped categories list", async () => {
    const user = userEvent.setup({ delay: null });

    renderPage(
      "/explore/exams/item/item-1?nav=act,math&keywords=act,math,algebra"
    );

    expect(
      await screen.findByText(/Flashcards-style study mode for reviewing quiz items/i)
    ).toBeInTheDocument();
