...
type TabId = "mine" | "bookmarked" | "shared" | "discover";
...
const CollectionsPage = () => {
  ...
  const [activeTab, setActiveTab] = useState<TabId>(tabParam && ["mine", "bookmarked", "shared", "discover"].includes(tabParam) ? tabParam : "mine");
  const [discoverQuery, setDiscoverQuery] = useState("");
  const [discoverPage, setDiscoverPage] = useState(1);
  const selectedCollectionRef = useRef<HTMLDivElement>(null);

  useEffect(() => {
    if (tabParam && ["mine", "bookmarked", "shared", "discover"].includes(tabParam)) {
  ...
  const { data: bookmarksData } = useQuery({
    ...
    enabled: isAuthenticated && !authLoading && (activeTab === "bookmarked" || activeTab === "shared"),
  });

  const { data: sharedData } = useQuery({
    queryKey: ["collections", "shared-with-me"],
    queryFn: () => collectionsApi.getSharedWithMe(),
    enabled: isAuthenticated && !authLoading && activeTab === "shared",
  });
  ...
  useEffect(() => {
    if (selectedCollectionId && (data?.collections?.length || bookmarksData?.collections?.length || sharedData?.collections?.length) && selectedCollectionRef.current) {
      setTimeout(() => {
        selectedCollectionRef.current?.scrollIntoView({ behavior: "smooth", block: "center" });
      }, 100);
    }
  }, [selectedCollectionId, data?.collections, bookmarksData?.collections, sharedData?.collections]);
  ...
  const handleCreateCollection = (e: React.FormEvent) => {
    e.preventDefault();
    if (collectionName.trim()) createMutation.mutate(collectionName.trim());
  };
    ...
    <div className="px-4 py-6 sm:px-0">
      <div className="mb-6">
        ...
        <p className="text-gray-600 text-sm">
          Your collections, bookmarks, shared with you, and discover public collections.
        </p>
      </div>

      <div className="flex flex-wrap items-center justify-between gap-4 mb-6">
        <div className="flex rounded-lg border border-gray-200 overflow-hidden bg-white">
          {(["mine", "bookmarked", "shared", "discover"] as const).map((tab) => (
            <button
              key={tab}
              type="button"
              onClick={() => setTab(tab)}
            ...
            >
              {tab === "mine" ? "Mine" : tab === "bookmarked" ? "Bookmarked" : tab === "shared" ? "Shared with me" : "Discover"}
            </button>
          ))}
        </div>
        {activeTab === "mine" && (
            ...
            <div className="mt-3">
              ...
              <form onSubmit={handleCreateCollection}>
                ...
                <div className="flex justify-end space-x-3">
                  <button
                    type="button"
                    onClick={() => { setShowCreateModal(false); setCollectionName(""); }}
      ...
      {activeTab === "shared" && (
        sharedCollections.length > 0 ? (
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {sharedCollections.map((c) => (
              <CollectionCard
                ...
                createdBy={c.createdBy}
                isOwner={false}
                isBookmarked={bookmarkedCollections.some((b) => b.id === c.id)}
                selectedCollectionId={selectedCollectionId}
                cardRef={selectedCollectionId === c.id ? selectedCollectionRef : undefined}
                onBookmark={bookmarkedCollections.some((b) => b.id === c.id) ? undefined : (id) => bookmarkMutation.mutate(id)}
                onUnbookmark={bookmarkedCollections.some((b) => b.id === c.id) ? (id) => unbookmarkMutation.mutate(id) : undefined}
          ...
          <div className="text-center py-12">
            <p className="text-gray-500">No collections shared with you yet.</p>
          </div>
        )
      )}
